...
#' @param column_suffix Logical indicating whether to add sampleId suffix to column name. If
...
#'
#' @examples NA
import_peak_annotation <- function(peakAnnoFile, sampleId = NULL, removePseudo = TRUE,
                                   fcCutoff = 1, pvalCutoff = 1,
                                   columns = NULL, column_suffix = TRUE){
  ...
  if(nrow(peakAnt) == 0){
    ...
    warning("No peaks detected by macs2 for sample: ", basename(peakAnnoFile), immediate. = TRUE)
